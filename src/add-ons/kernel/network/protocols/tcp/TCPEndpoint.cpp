...
TCPEndpoint::ReadData(size_t numBytes, uint32 flags, net_buffer** _buffer)
{
	...
	// DIAG-E2: probe fLock (the RX-consumer-shared lock this study is about).
	// Post-#323 this critical section is split: the O(segments) coalesce runs
	// with fLock RELEASED (locker.Unlock()/Lock() around the ring drain below),
	// so the probe now measures the *reduced* hold that is the #61 win itself.
	// fReadLock is a separate per-reader lock and is intentionally left as a
	// plain MutexLocker -- it is not the lock this instrument targets.
	TCPFlockProbe locker(fLock);
