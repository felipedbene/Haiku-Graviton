...
| `harfbuzz-14.2.0.recipe` | yes, **INCOMPLETE** | docs cut (`-Ddocs=disabled`, dropped `pygments_$pythonPackage` and `cmd:gtkdoc_scan` → `gtk_doc` → `pygments_python310`, unbuildable here), and gobject/introspection cut (`-Dgobject=disabled -Dintrospection=disabled`, removed the `harfbuzz_glib` subpackage + its `libglib_2.0`/`libgobject_2.0`/`libgirepository_1.0` build-reqs) to avoid pulling **glib2**, which is blocked (see status doc). **Still fails**: with full `freetype-2.14.3` built, harfbuzz's meson still does not put freetype's include dir on the `hb-ft.h` compile (`ft2build.h: No such file`) — an unresolved chroot freetype2 pkg-config detection issue. ISP recipe (input-source-packages), pin mtime forward |
