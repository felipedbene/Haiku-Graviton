...
| `agent-sops/debeos-metal-build-debug.sop.md` | **Metal build-debug** (fast `jam` iteration): reproduce a CrossBuild/package-solve failure on the c9g.metal builder and fix it in minutes instead of one bake at a time. The "verify it builds" loop between staging a change and the hardware-proof bake. Documents the traps (clone-trap tree, HAIKU_REVISION, proper local-package staging, the sudo/PWD jam trap). |
