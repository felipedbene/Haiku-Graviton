...
## Where this stands — **2026-08-24 15:05Z**
...
|---|---|---|
| **ports built natively** | **47** | distinct recipes we have built on arm64 |
| **hpkgs produced natively** | **98** | package files those ports emitted (base + `_devel`/`_debuginfo`/`_doc`/…) |
| **`.hpkg` files in `hpkg-out/arm64/`** | **134** | the 98 above **+ 28** cross-built `_bootstrap` inputs **+ 8** chroot inputs (`haiku*.hpkg`, `makefile_engine`, `netfs`, `userland_fs`) — build *inputs*, not our output |
| **files under `hpkg-out/` entirely** | **~270** | the above **plus two other directories**: `arm64-nondirty/` (the shared build pool, a duplicate) and `arm64-dirty-20260824/` (the 52-file pre-clock-fix snapshot kept as evidence). Mostly duplicates and superseded files |
...
this session added 9 more ports (**`autoconf_archive`, `libedit`, `libffi`, `libpng16`,
`nasm`, `python3.10`, `python3.14`, `file`** — see Blocker 9).
...
path). `groff` is now **17 ports / 10 waves** away with **no cuts**; the next four ports
are all immediately buildable. See Blocker 9.
...
## Blocker 9 — the rest of the chain: deep, but **no cycles anywhere** — **OPEN**
...
|---|---|---|---|---|
| `python3.10`/`python3.14` | no | **0 — DONE** | the PGO/LTO cut + additive `RUNSHARED` | both built this session |
| PEP-517 ladder → `meson`/`ninja` | no | 7 | none | strictly serial, small builds |
| `groff` | no | **17** | **none** | `jasper`/`netpbm`, moderate |
| `netsurf` | no | ~45 | **none** | breadth, not depth |
| `haikuwebkit` | no | ~27 | **2** (rav1e codec, libpsl) | **LLVM ≥ 21**, hours |
...
| groff 1.23.0 | **not blocked — 17 ports away, no cuts** | needs `cmd:pnmcrop`/`pnmtopng`/`pnmtops` (netpbm) and `cmd:psselect` (psutils). **Not a cycle**, and the mesa/LLVM/`cmd:git` story was wrong: see **Blocker 9**, which supersedes "Why a real groff is still out of reach" below |
