...
| **IFUNC / target_clones** | `--enable-ifunc`, `HAVE_IFUNC`, `__attribute__((ifunc))`, `target_clones`, `__builtin_cpu_supports` | the `.so` **fails to load silently** on Haiku arm64 (`R_AARCH64_IRELATIVE` → `B_BAD_DATA`, diagnostic compiled out). No runtime SIMD dispatch exists on this port. | disable it; decide SIMD at compile time |
