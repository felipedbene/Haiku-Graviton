...
## Hardware questions: search Amazon-internal sources FIRST

**Standing rule.** For any question about how the *hardware or firmware* behaves —
ACPI tables, PCIe/ECAM topology, GIC and ITS, ENA capabilities, instance limits,
Nitro behaviour, SoC layout — **search Amazon-internal sources before designing an
experiment.** Use the `builder-mcp` tools: `InternalCodeSearch` (semantic first, then
`code` for exact strings), `InternalSearch` (try `WIKI`, `BROADCAST`, `SAGE_HORDE`),
and `ReadInternalWebsites` for any `*.amazon.com` / `a2z.com` / `aws.dev` URL.

**Why this is the rule and not a suggestion.** It is faster and cheaper than booting
hardware, and it yields facts that empirical work on one instance *cannot* produce.
Worked examples from this project:

- The ENA bring-up problem was unblocked by reading Amazon's own driver sources
  rather than by black-box experiment.
- The `c8g.24xlarge` three-region MCFG looked like a firmware quirk. Internal search
  established that **Graviton 4 has two PCIe dies with four controllers each**, which
  reframes those bus ranges as potentially *physical* partitions rather than an
  arbitrary way of writing one window — and therefore changes what the fix has to be
  safe against. No single boot log implies that.
- The same search found a captured Graviton 3 boot log with a **two**-allocation MCFG,
  establishing that multi-allocation MCFG is normal AWS practice across generations
  rather than one class's bug — which argues for handling N regions generically.

**Know the limits, so search failure is not mistaken for an answer.** The Nitro /
Graviton firmware *source* does not appear to be in the searchable index. What is
reachable and useful:

| what | where |
|---|---|
| real boot logs / ACPI dumps from Graviton hardware | test-data and golden-data files in other teams' repos — search for `GRVTN` or an OEM table ID |
| firmware architecture and design intent | `BROADCAST` (training sessions), `WIKI` |
| other teams' platform firmware, for comparison | EDK2 / DynamicTablesPkg copies |
| ACPI/PCI spec structures and field semantics | vendored ACPICA copies |

**It complements hardware verification, it does not replace it.** Search establishes
what the platform is *specified or intended* to do; hardware establishes what it
actually does, and this project has been bitten both ways — a QEMU proxy diverged from
EC2 on the power button (GED vs PL061 GPIO), and a plausible reading of a spec was
overturned by a register readback (`GICD_CTLR` bit 0 is write-ignored on both AWS
platforms). **So: search first to know what you are looking at, then measure to know
what you have.** A search result is a hypothesis with a good pedigree, not a
measurement — label it accordingly.
